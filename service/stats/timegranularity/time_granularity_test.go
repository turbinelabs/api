...
func TestTimeGranularityString(t *testing.T) {
	assert.Equal(t, Seconds.String(), "seconds")
	assert.Equal(t, Unknown.String(), "unknown(3)")
...
func TestIsValid(t *testing.T) {
	...
	for _, tg := range invalid {
		assert.False(t, IsValid(tg))
	}

	ForEach(func(tg TimeGranularity) {
		assert.True(t, IsValid(tg))
	})
}
