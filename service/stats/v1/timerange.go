...
import (
	"github.com/turbinelabs/api/service/stats/v1/timegranularity"
)

// TimeRange specifies a range of time over which stats queries are
// performed.
type TimeRange struct {
	...
	// return. In the v1.0 API, it is specified in microseconds. In
	// the v2.0 API, it is specified in seconds. End takes precedence
	// over Duration. If Start is specified, Duration sets the end of
	// the time span (e.g. from Start for a period of Duration). If
	// Start is not specified, Duration sets the start of the time
	// span that period into the past (e.g., a period lasting
	// Duration, until now).
...
// SimpleTimeRange represents the start and end of a time range. In
// the v1.0 API, times are specified in microseconds since the Unix
// epoch, UTC.
