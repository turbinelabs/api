...
func TestStatsV2ClientForwardWrongVersion(t *testing.T) {
	ctrl := gomock.NewController(assert.Tracing(t))

	mockExec := executor.NewMockExecutor(ctrl)

	client, err := NewStatsV2Client(endpoint, clientTestAPIKey, clientTestApp, mockExec)
	assert.Nil(t, err)

	result, err := client.(statsapi.StatsService).Forward(nil)
	assert.Nil(t, result)
	assert.ErrorContains(t, err, "cannot use Forward on v2 client")
}
